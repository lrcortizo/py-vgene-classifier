...
def print_report(pred_df, validated, misclassified, no_match, ref_counts):
    ...
    if with_match > 0:
        ...
        print(f"\nPrecision (of matched predictions): {precision:.1f}%")

    # Per-locus breakdown
    print("\n" + "-"*70)
    print("PER-LOCUS BREAKDOWN:")
    print("-"*70)
    print(f"{'Locus':<10} {'Predicted':<12} {'Validated':<12} {'Missed (IMGT)':<15} {'Recall':<10}")
    print("-"*70)

    for locus in ['IGHV', 'IGKV', 'TRAV', 'TRBV']:
        n_pred = len(pred_df[pred_df['predicted_locus'] == locus])
        n_val = len(validated[validated['predicted_locus'] == locus])
        n_imgt = ref_counts.get(locus, 0)

        recall = (n_val / n_imgt * 100) if n_imgt > 0 else 0
        missed = n_imgt - n_val

        print(f"{locus:<10} {n_pred:<12} {n_val:<12} {missed:<15} {recall:.1f}%")
...
def main():
    ...
    print_report(vgenes_only, validated, misclassified, no_match, ref_counts)
